...
public class EnterprisemsgActivity extends Activity implements OnClickListener {
	...
	public static final int MSG_UPDATE_PLAYED_STATUS = 402;
	...
	/* the flag for going to play */
	private boolean mbPlayPending = false;

	private final TtsStatusListener mTtsStatusLsnr = new TtsStatusListener() {
		public void onTtsDone(String id) {
			if (mbPlayPending) {
				mbPlayPending = false;
				if (mPlayIndex > -1) {
					if (ttsSpeak(mEPMsgItemList.get(mPlayIndex))) {
						updateReadFlag(mPlayIndex);
					} else {
						mPlayIndex = -1;
					}
				}
			...
			msg.what = MSG_UPDATE_PLAYED_STATUS;
			msg.arg1 = mPlayIndex;
			mMsgHandle.sendMessage(msg);
		}
	};
	...
	private void showEPMsgDeatil(EPMessageItem item, int pos) {
		...
		intent.putExtra(MsgDeatilActivity.EPMSG_ITEM_LIST,
				(ArrayList<EPMessageItem>) mEPMsgItemList);
		mPosIndex = pos;
	...
	private class MyMsgListAdapter extends ArrayAdapter<EPMessageItem> {
		...
		public View getView(int position, View convertview, ViewGroup parent) {
					...
					if (mbPlayPending == false) {
						if (mPlayIndex != -1) {
							...
							if (ttsSpeak(info)) {
								...
								mTtsMgr.setTtsListener(mTtsStatusLsnr);
							}
						}
					}
				}
			...
			if (mPlayIndex == index && mPlayIndex >= 0) {
				btnttsplay.setVisibility(View.GONE);
				btnttsstop.setVisibility(View.VISIBLE);
			} else {
				btnttsplay.setVisibility(View.VISIBLE);
	...
	private class MsgHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case MSG_UPDATE_PLAYED_STATUS: {
				mEPMsgListAdapter.notifyDataSetInvalidated();
				break;
			}
			default:
				break;
